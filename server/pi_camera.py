...
local_storage_path = ""  # Initialize as empty string instead of None
    ...
    try:
        ...
        config = camera.create_video_configuration(
            ...
            controls={
                ...
                "AwbMode": controls.AwbModeEnum.Auto   # Set AWB mode to Auto
            }
        )
        ...
        logger.info(f"Camera initialized: {width}x{height} @ {framerate}fps with Auto White Balance")
        return True
...
def start_continuous_recording(segment_minutes=DEFAULT_SEGMENT_MINUTES):
   ...
   # Check if local_storage_path is set
   if not local_storage_path:
       logger.error("Local storage path not initialized")
       return False
...
def continuous_recording_thread(segment_minutes):
   ...
   try:
       while recording:
           ...
           if not local_storage_path:
               logger.error("Local storage path not set during recording")
               break
...
def capture_image_endpoint():
   try:
       ...
       # Check if this is a light reference capture
       section = data.get('section')  # 'day' or 'night'
       image_type = data.get('imageType')  # 'on' or 'off'
      
       # Create filename based on type of capture
       if section and image_type:
           # Light reference image
           filename = f"light_ref_{section}_{image_type}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
       else:
           ...
           if connected_to_server:
               try:
                   ...
                   with open(output_path, 'rb') as img_file:
                       ...
                       # Add light reference metadata if applicable
                       if section and image_type:
...
# Dedicated endpoint for light reference captures (wrapper around main capture)
@app.route('/capture-light-reference', methods=['POST'])
def capture_light_reference():
   """Dedicated endpoint for capturing light reference images"""
   try:
       from flask import request
      
       # Parse request data
       data = request.json if request.is_json else {}
       section = data.get('section', 'day')  # 'day' or 'night'
       image_type = data.get('imageType', 'on')  # 'on' or 'off'
      
       # Create the request data for the main capture function
       capture_data = {
           'name': f'light_reference_{section}_{image_type}',
           'timestamp': datetime.datetime.now().isoformat(),
           'section': section,
           'imageType': image_type
       }
      
       # Create a mock request object for the capture function
       class MockRequest:
       ...
       mock_request = MockRequest(capture_data)
      
       # Call the main capture function logic
       try:
           # Parse request data
           name = capture_data.get('name', 'unknown')
           timestamp = capture_data.get('timestamp', datetime.datetime.now().isoformat())
          
           # Create filename for light reference image
           filename = f"light_ref_{section}_{image_type}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg"
           ...
           if camera:
               ...
               logger.info(f"Light reference image captured: {output_path}")
               ...
               if connected_to_server:
                   try:
                       ...
                       with open(output_path, 'rb') as img_file:
                           ...
                           response = requests.post(
                               f"{server_url}/upload-light-reference",
                               files=files,
                               data={'metadata': json.dumps(metadata)}
                           )
                           ...
                           if response.status_code == 200:
                               result = response.json()
                               server_image_url = result.get('image_url')
                               logger.info(f"Light reference uploaded to server: {server_image_url}")
                           else:
                               logger.error(f"Failed to upload light reference: {response.status_code}")
                   except Exception as e:
                       logger.error(f"Error uploading light reference: {e}")
               ...
               logger.error("Camera not available for light reference capture")
               return jsonify({
                   'success': False,
           ...
           logger.error(f"Error in light reference capture: {inner_e}")
           return jsonify({
               'success': False,
               'error': str(inner_e)
           }), 500
       ...
       logger.error(f"Error capturing light reference: {e}")
       return jsonify({
           'success': False,
           'error': str(e)
       }), 500
